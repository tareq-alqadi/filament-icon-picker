...
<div
	...
	->class(['filament-icon-picker-icon-column', 'px-3 py-4' => !$isInline()]) }}>
    ...
    @if ($icon = $getState())
        ...
        <x-filament::icon :icon="$icon" @class([
            ...
            match ($size) {
                ...
                IconColumnSize::ExtraExtraLarge,
                '2xl'
                    => 'fi-ta-icon-item-size-2xl h-8 w-8',
                default => $size,
            },
            match ($color) {
            ...
            \Filament\Support\get_color_css_variables($color, shades: [400, 500], alias: 'tables::columns.icon-column.item') => $color !== 'gray',
        ]) />
